...
const ArrivalForm = ({
  ...
  const [promptData, setPromptData] = useState({
    farmName: "",
    farmCode: "",
    dateOfPacking: initialFormState.dateOfPacking,
  });
  ...
  const getSamplingRequirement = (farmCode, datePacked) => {
    ...
    const alreadySampled = samplings.find(
      (s) =>
        s.farmCode === farmCode &&
        (s.date === datePacked || s.dateOfPacking === datePacked)
    );
    if (alreadySampled) {
      return {
        isRequired: false,
        status: alreadySampled.overallDecision || alreadySampled.decision,
      };
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !formData.farmName ||
      !formData.farmCode ||
      !formData.dateOfPacking ||
      !formData.dateTimeArrive
    ) {
      setFormError(
        "Validation Error: Missing core date or farm data! Please ensure Farm Name and Dates are complete."
      );
      return;
    }

    if (
      !formData.deliveryReceipt ||
      !formData.driverName ||
      !formData.plateNumber ||
      !formData.brand
    ) {
      setFormError(
        "Validation Error: Please complete all Delivery Header fields (Delivery Receipt, Driver, Plate, Brand) before saving."
      );
      return;
    }
    ...
    if (!formData.packaging || !formData.piw) {
      setFormError(
        "Validation Error: Please select the Packaging and Pack-in-Weight."
      );
      return;
    }
    ...
    if (!farmObj) {
      setFormError(
        "Critical Error: Farm validation failed. Please re-select the Farm Name."
      );
      return;
    }
    ...
    const activeRateDoc = weeklyRates.find(
      (r) =>
        r.farm_id === farmObj.id &&
        r.year === packingYear &&
        r.week_number === packingWeek
    );
    ...
    const packCode = formData.packaging
      ? formData.packaging.charAt(0).toUpperCase()
      : "V";
    const weightCode = formData.piw
      ? formData.piw.toString().replace(".", "")
      : "135";
    ...
    addLine(
      ...
      "FP",
      "FP",
      formData.classB.shbFP
    );

    // Validate if at least ONE box was entered across all inputs
    if (newArrivalsBatch.length === 0) {
      alert(
        "⚠️ Validation Error: You must enter at least 1 box quantity in the Grid before saving the batch."
      );
      return;
    }
      ...
      <div className="animation-fade-in">
        ...
        >
          <div>
            ...
            >
              View and manage daily banana deliveries from farms.
            </p>
          </div>
          <button
            className="btn-primary"
            onClick={() => setShowFarmPrompt(true)}
        ...
        {showFarmPrompt && (
          ...
          >
            ...
            >
              <button
                onClick={() => setShowFarmPrompt(false)}
                style={{
                  position: "absolute",
                  top: "15px",
                  right: "15px",
              ...
              >
                Select the farm and packing date below. Deliveries cannot be
                logged without a cleared sample for the day.
              </p>
              ...
              <div className="form-group" style={{ marginBottom: "1rem" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handlePromptChange("dateOfPacking", e.target.value)
                  }
                />
              </div>
              ...
              <div className="form-group" style={{ marginBottom: "1rem" }}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handlePromptChange("farmName", e.target.value)
                  }
                >
              ...
              {promptData.farmCode && promptSamplingState && (
                <div
                  ...
                  style={{
                    ...
                    padding: "1.25rem",
                    borderRadius: "8px",
                    backgroundColor: promptSamplingState.isRequired
                      ? "#fffbeb"
                ...
                >
                  {promptSamplingState.isRequired ? (
                    <>
                      ...
                      >
                        This farm has no logged sampling for{" "}
                        {promptData.dateOfPacking}. Please evaluate a sample
                        container first.
                      </p>
                      <button
                        className="btn-primary"
                        ...
                        onClick={() =>
                          onNavigate({
                            name: "sampling",
                            state: { farmCode: promptData.farmCode },
                          })
                        }
                      >
                    ...
                    <>
                      ...
                      >
                        This farm's sample was officially rejected. Operations
                        are normally suspended, but you may proceed to log
                        specific accepted boxes if authorized.
                      </p>
                      <button
                        type="button"
                    ...
                    <>
                      ...
                      >
                        Decision: <strong>{promptSamplingState.status}</strong>
                      </p>
                      <button
                        className="btn-primary"
                        style={{
    ...
    <div className="card form-container container-slide-down">
      ...
      >
        <div>
          ...
          <p className="subtitle">
            Enter the delivery headers below. The item grid will automatically
            be broken down into individual inventory specs.
          </p>
        </div>
        <button
          className="btn-secondary"
      ...
      <form onSubmit={handleSubmit} className="arrival-form">
          ...
          <div className="grid-3">
            <div className="form-group">
              <label className="label">
                Farm Name <span style={{ color: "red" }}>*</span>
              </label>
              <select
                name="farmName"
                className="input-field"
            ...
            <div className="form-group">
              <label className="label">
                Delivery Receipt # <span style={{ color: "red" }}>*</span>
              </label>
              <input
                type="text"
            ...
            <div className="form-group">
              <label className="label">
                Brand Prefix <span style={{ color: "red" }}>*</span>
              </label>
              <input
                type="text"
                name="brand"
            ...
            <div className="form-group">
              <label className="label">
                Pack in Weight (kg) <span style={{ color: "red" }}>*</span>
              </label>
              <input
                type="number"
                step="0.1"
            ...
            <div className="form-group">
              <label className="label">
                Date & Time Arrived <span style={{ color: "red" }}>*</span>
              </label>
              <input
                type="datetime-local"
                name="dateTimeArrive"
                className="input-field"
                value={formData.dateTimeArrive}
                onChange={handleChange}
              />
            </div>
            <div className="form-group">
              <label className="label">
                Date of Packing <span style={{ color: "red" }}>*</span>
              </label>
              <input
                type="date"
                name="dateOfPacking"
        ...
        {samplingState.isRequired && (
          <div
            className="form-section highlight-yellow slide-down"
            style={{ borderLeft: "4px solid var(--warning)" }}
          >
            <h3>Mandatory Daily Sampling Required</h3>
            <p
              className="subtitle"
              style={{ color: "var(--warning-dark)", marginTop: "0.5rem" }}
            >
              {formData.farmName} has not been sampled today. You must evaluate
              a first-delivery sample before logging inventory.
            </p>
            ...
            >
              <button
                ...
                onClick={() =>
                  ...
                  onNavigate({
                    name: "sampling",
                    state: { farmCode: formData.farmCode },
                  })
                }
              >
                Go to Daily Sampling
        ...
        {samplingState.status === "REJECTED" && !samplingState.isRequired && (
          ...
          >
            ...
            <p>
              A sample from this farm was evaluated and rejected for today. You
              may still proceed to log inventory for the remaining accepted
              boxes.
            </p>
            <button
              type="button"
        ...
        <div
          className={`form-section breakdown-section highlight-green ${
            samplingState.status === "DOWNGRADED" || samplingState.isRequired
              ? "disabled-section"
              : ""
          }`}
          style={{
        ...
        >
          <div className="section-header-flex">
            <h3>
              Class A{" "}
              {samplingState.status === "DOWNGRADED" && (
                <span
                  style={{
                    color: "var(--danger)",
                    fontSize: "0.9rem",
        ...
        <div
          className={`form-section breakdown-section highlight-yellow ${
            samplingState.isRequired ? "disabled-section" : ""
          }`}
          style={{
            opacity: samplingState.isRequired ? 0.5 : 1,
            pointerEvents: samplingState.isRequired ? "none" : "auto",
          }}
        >
          <div className="section-header-flex">
            <h3>Class B</h3>
            <span className="section-total">
              Total: {totals.classBTotal} bxs
            </span>
          </div>
        ...
        >
          ...
          <div className="btn-group">
            ...
            <button
              type="submit"
              className="btn-primary"
              disabled={totals.grandTotal === 0 || samplingState.isRequired}
            >
              Save Batch Arrival
            </button>
          </div>
