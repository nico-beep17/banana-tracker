                    ...
                    <div style={{ padding: '15px' }}>
                        <table style={{ width: '100%', fontSize: '14px', borderCollapse: 'collapse' }}>
                            <tbody>
                                ...
                                <tr><td style={{ padding: '6px 0', color: '#64748b' }}>Driver:</td><td style={{ fontWeight: '700', padding: '6px 0' }}>{container.driver || 'N/A'}</td></tr>
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: '14px', marginBottom: '20px', border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}>
                <thead>
                    <tr style={{ backgroundColor: '#f8fafc' }}>
                        ...
                        <th style={{ padding: '12px 15px', textAlign: 'left', borderBottom: '1px solid #e2e8f0', color: '#475569', fontWeight: '700' }}>Description / Logger</th>
                ...
                <tbody>
                    {container.stuffedItems && container.stuffedItems.length > 0 ? (
                        container.stuffedItems.map((item, index) => (
                            <tr key={index}>
                                <td style={{ padding: '10px 15px', borderBottom: index === container.stuffedItems.length - 1 ? 'none' : '1px solid #f1f5f9' }}>{formatDate(item.timestamp)}</td>
                                <td style={{ padding: '10px 15px', borderBottom: index === container.stuffedItems.length - 1 ? 'none' : '1px solid #f1f5f9', color: '#64748b' }}>System Container Stuffing Entry</td>
                                <td style={{ padding: '10px 15px', textAlign: 'right', fontWeight: '700', borderBottom: index === container.stuffedItems.length - 1 ? 'none' : '1px solid #f1f5f9' }}>{item.total.toLocaleString()}</td>
                            </tr>
                        ))
                    ) : (
                        <tr>
                            <td colSpan="3" style={{ padding: '30px', textAlign: 'center', color: '#94a3b8', fontStyle: 'italic' }}>No box entries recorded for this container.</td>
                        </tr>
                    )}
                </tbody>
            </table>
